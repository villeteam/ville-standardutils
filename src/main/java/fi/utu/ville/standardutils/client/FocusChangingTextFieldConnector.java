...
//	private Logger log = Logger.getLogger("Focus");
	...
	private KeyPressHandler keyPressHandler = new KeyPressHandler() {
	    ...
	    public void onKeyPress(KeyPressEvent event) {
	        ...
	        String newText = RegexFieldExtensionConnector.getFieldValueAfterKeyPress(textField, charCode);
	        
	        
	        boolean isFull = newText.length() >= getState().getChangeAfter();
	        
//	        log.log(Level.SEVERE, "Keypress: " + oldText + "," + newText + "," + charCode + "," + cursorPos + "," + selectionLength + "," + isFull);
	        
	        FocusAction focusAction = logic.keyPress(oldText, newText, charCode, cursorPos, selectionLength, isFull);
...
//	    }
		textField.addKeyDownHandler(new KeyDownHandler() {
			...
			public void onKeyDown(KeyDownEvent event) {
				...
				if(event.getNativeKeyCode() == KeyCodes.KEY_BACKSPACE) {
					FocusAction action = logic.backspacePress(textField.getText(), RegexFieldExtensionConnector.getFieldValueAfterKeyPress(textField, '\b'), textField.getCursorPos(), textField.getSelectionLength());
					executeAction(action);
				}
				
				Direction arrowDirection = Direction.keycodeToDirection(event.getNativeKeyCode());
				if(arrowDirection != Direction.THIS) {
					FocusAction action = logic.arrowPress(arrowDirection, textField.getCursorPos(), textField.getText());
					executeAction(action);
	...
	private void executeAction(FocusAction focusAction) {
        if(focusAction.hasTextChangeCommands()) {
        	for(Map.Entry<Direction, String> textChange : focusAction.getTextChangeCommands().entrySet()) {
        		VVilleTextField field = state.getComponentAtDirection(textChange.getKey());
        		if(field != null) {
        			field.cancelKey();
	        ...
	        public void execute () {
	        	...
	        	if(widget instanceof VVilleTextField) {
	    			...
	    			if(field.getText().length() > 0) {
		    			...
		    			int cursorPos = focusAction.getActualCursorPos(field.getText());
...
//		    				log.log(Level.SEVERE, "Changing selection");
		    				...
		    				field.setSelectionRange(cursorPos, focusAction.getActualSelectionLength(cursorPos, field.getText()));
		    			}
	    			}
	    		}
	        }
