 ...
 * 
 * 
 * @author Erkki Kaila, Riku Haavisto, Niko Hellgren
 * 
 */
...
public class StandardIcon {
	...
	public enum IconVariant {
		...
		LIST("fa-li"),
		/**
		 * Spinning (animated) icon. Most useful with icons such as {@link Icon#spinner}.
		 */
		SPIN("fa-spin"),
		/**
		...
		BORDER("fa-border"),
		/**
		 * For e.g for easy pull quotes or article graphics, with larger icons next to text.
		 */
		PULL_LEFT("pull-left"),
		/**
		 * For e.g for easy pull quotes or article graphics, with larger icons next to text.
	 ...
	 * Font-Awesome icon.
	 * <p>
	 * The basic {@link #toString()} will produce HTML representing the regular icon.<br/>
	 * For variants, {@link #variant(IconVariant...)} can be used with one or more {@link IconVariant}s.<br/>
	 * For custom cases, the stylename is available via {@link #stylename()}.
	 * </p>
	 * <p>
	 * Remember to {@link FontAwesome#load()} (or {@link FontAwesome#load(Resource)}) first.
	 * </p>
	 * 
	 * @author marc
	 * 		
	...
	public enum RawIcon {
		...
		/**
		 * Gets the Unicode codepoint for given character
		 * 
		 * @return
		 */
		 ...
		 * <p>
		 * Multiple {@link IconVariant}s can be used, but all combinations do not make sense.
		 * </p>
		 * 
		 * @param variants
		 * @return
		...
		public String variant(Color color, IconVariant... variants) {
			return variant(null, color, variants);
		}
	...
	/**
	 * Gets the HTML code for a modified FontIcon
	 * 
	 * @param icon
	 * @param color
	 * @param customClass
	 ...
	 * @param iconVariants
	 * @return String representing the icon with wanted properties in a HTML {@code <span>}
	 */
	public static String getModifiedIconHtml(FontIcon icon, Optional<Color> color, Optional<String> customClass, IconVariant... iconVariants) {
		if (icon instanceof Icon) {
			return ((Icon) icon).getIcon().variant(color.orElse(null), iconVariants);
		}
		return spanWrapper(icon.getHtml(), customClass.orElse(""), color, iconVariants);
	}
	
	private static String spanWrapper(String iconHtml, String styles, Optional<Color> color, IconVariant... iconVariants) {
		String colorString = "";
		String stylenames = styles;
		
		for (IconVariant v : iconVariants) {
			stylenames += " " + v;
		}
		if (color.isPresent()) {
			colorString = " style=\"color: " + color.get().getCSS() + ";\"";
		}
		return "<span class=\"" + stylenames + "\"" + colorString + ">" + iconHtml + "</span>";
	}
	
	/**
	 * Usage of {@link edu.vserver.main.Icon7.IconConstants} recommended if wanted icon is only used in VServer
	 */
	public enum Icon implements FontIcon {
		LOG_IN(RawIcon.sign_in),
		PHONE(RawIcon.phone),
		MENU(RawIcon.reorder),
		USER(RawIcon.user),
		HELP(RawIcon.question),
		CHANGE_ROLE(RawIcon.columns),
		LOG_OUT(RawIcon.sign_out),
		SETTINGS(RawIcon.cog),
		NAVIGATOR_ARROW_LEFT(RawIcon.chevron_left),
		NAVIGATOR_ARROW_RIGHT(RawIcon.chevron_right),
		ERROR(RawIcon.times_circle_o),
		EXIT(RawIcon.arrow_circle_left),
		SUBMIT(RawIcon.upload),
		RESET(RawIcon.refresh),
		WINDOW_MINIMIZE(RawIcon.compress),
		WINDOW_MAXIMIZE(RawIcon.expand),
		WINDOW(RawIcon.external_link),
		COURSE(RawIcon.book),
		ROUND(RawIcon.th_list),
		OK(RawIcon.check),
		STARTED(RawIcon.circle_o),
		NOT_STARTED(RawIcon.circle),
		MINIMUM_ACHIEVED(RawIcon.circle),
		DEADLINE(RawIcon.clock_o),
		CALENDAR(RawIcon.calendar),
		STUDY_JOURNAL(RawIcon.edit),
		MANUAL_OTHER(RawIcon.pencil),
		DONE(RawIcon.check),
		DEMONSTRATION(RawIcon.check),
		ATTENDANCE(RawIcon.clock_o),
		EXAM(RawIcon.tasks),
		COURSE_ASSIGNMENT(RawIcon.copy),
		REGISTRATION(RawIcon.check_circle_o),
		BOOKING_CALENDAR(RawIcon.calendar),
		AUTOMATIC_ASSIGNMENT(RawIcon.upload),
		WARNING(RawIcon.exclamation),
		NEWS(RawIcon.info),
		CLOSE(RawIcon.times),
		SEPARATOR_CARET(RawIcon.angle_double_right),
		PROFILE(RawIcon.user),
		MESSAGES(RawIcon.envelope_o),
		INBOX(RawIcon.inbox),
		SENT_MAIL(RawIcon.share),
		OWN_RESOURCES(RawIcon.star),
		PUBLIC_RESOURCES(RawIcon.th),
		EXERCISE(RawIcon.edit),
		MATERIALS(RawIcon.image),
		TUTORIAL(RawIcon.file_o),
		STUDENTS(RawIcon.group),
		STATISTICS(RawIcon.bar_chart_o),
		STUDENT_PERFORMANCE(RawIcon.tasks),
		MANUALLY_GRADED(RawIcon.check),
		PRACTICAL_WORK(RawIcon.flask),
		FOLD(RawIcon.minus),
		UNFOLD(RawIcon.plus),
		BOOKMARK(RawIcon.bookmark),
		CONTACTS(RawIcon.tags),
		PASSWORD(RawIcon.lock),
		SAVE(RawIcon.save),
		DELETE(RawIcon.trash_o),
		REVIEW(RawIcon.check),
		GROUPS(RawIcon.group),
		EVENTS(RawIcon.clock_o),
		TEACHER(RawIcon.user),
		NAVIGATOR_ARROW_FIRST(RawIcon.step_backward),
		NAVIGATOR_ARROW_LAST(RawIcon.step_forward),
		PANEL_FOLD(RawIcon.sort_down),
		PANEL_UNFOLD(RawIcon.caret_right),
		SEARCH(RawIcon.search),
		NEW_RESOURCE(RawIcon.file),
		EDIT(RawIcon.edit),
		FOLDER(RawIcon.folder_o),
		TEST(RawIcon.play),
		COPY(RawIcon.copy),
		REPLY(RawIcon.reply),
		NEW(RawIcon.file),
		CANCEL(RawIcon.times),
		MATH_CHECK(RawIcon.check),
		MATH_NEXT(RawIcon.chevron_right),
		MATH_PREV(RawIcon.chevron_left),
		INFO(RawIcon.info),
		TAGS(RawIcon.tags),
		GENERATE(RawIcon.refresh),
		ADD_ASSIGNMENT(RawIcon.upload),
		ADD(RawIcon.plus),
		CHECK_VALIDITY(RawIcon.refresh),
		BROWSE_THUMBS(RawIcon.th),
		BROWSE_TABLE(RawIcon.table),
		MATH_LEVEL_EASY(RawIcon.star),
		STAR(RawIcon.star),
		MATH_LEVEL_NORMAL(RawIcon.star),
		MATH_LEVEL_HARD(RawIcon.star),
		ROUND_VISIBLE(RawIcon.eye),
		ROUND_HIDDEN(RawIcon.eye_slash),
		UP(RawIcon.arrow_up),
		DOWN(RawIcon.arrow_down),
		PREVIEW(RawIcon.eye),
		EXERCISE_NAME(RawIcon.user),
		EXERCISE_DESCRIPTION(RawIcon.edit),
		NOTES_FOR_TEACHERS(RawIcon.envelope),
		HIDE(RawIcon.eye_slash),
		MONITOR(RawIcon.desktop),
		CONDITIONAL_ROUND(RawIcon.list_ol),
		DOWNLOAD(RawIcon.download),
		ENROLL(RawIcon.sign_in),
		UPDATE(RawIcon.refresh),
		RFID(RawIcon.square),
		TEMPLATE(RawIcon.signal),
		CALCULATE_TOTALS(RawIcon.plus_circle),
		SURVEY(RawIcon.paste),
		LIBRARY(RawIcon.list),
		TEXT(RawIcon.file_o),
		IMAGE(RawIcon.image),
		AUDIO(RawIcon.volume_up),
		MOVIE(RawIcon.film),
		OBJECT(RawIcon.suitcase),
		TABLE(RawIcon.table),
		LAYOUT_SINGLE(RawIcon.square_o),
		LAYOUT_DOUBLE(RawIcon.columns),
		LAYOUT_2X2(RawIcon.th_large),
		LAYOUT_3X3(RawIcon.th),
		LAYOUT_HEADER(RawIcon.h_square),
		LAYOUT_PAGE_BREAK(RawIcon.share_alt),
		LAYOUT_TRIPLE(RawIcon.columns),
		UPLOAD(RawIcon.upload),
		SIZE(RawIcon.expand),
		SORT_OPTIONS(RawIcon.tasks),
		STYLE_SETTINGS(RawIcon.magic),
		SORT(RawIcon.sort),
		FEEDBACK(RawIcon.comment_o),
		SURVEY_OPTIONS_ICON(RawIcon.circle_o),
		SURVEY_BOOLEAN_ICON(RawIcon.check),
		SURVEY_TEXTFIELD_ICON(RawIcon.minus),
		SURVEY_TEXTAREA_ICON(RawIcon.list_alt),
		SURVEY_SCALE_ICON(RawIcon.exchange),
		SURVEY_EXPLANATION_ICON(RawIcon.question),
		SURVEY_UPLOAD_ICON(RawIcon.upload),
		SURVEY_BREAK_ICON(RawIcon.cut),
		SURVEY_OPTIONS_OPEN(RawIcon.comment),
		SURVEY_GRID_ICON(RawIcon.th),
		MIME_IMAGE_MEDIUM(RawIcon.image),
		MIME_MOVIE_MEDIUM(RawIcon.video_camera),
		IMAGE_TAG(RawIcon.tags),
		SURVEY_COMPONENTS(RawIcon.th_large),
		TEXTFIELD(RawIcon.list_alt),
		MCQ_PRECENTAGE_SELECT_ONE(RawIcon.plus_circle),
		MCQ_COMBINED_PERCENTAGE(RawIcon.plus_square),
		SHORT_ANSWER(RawIcon.font),
		MCQ(RawIcon.list),
		MCQ_MATH(RawIcon.thumb_tack),
		QUESTIONS(RawIcon.question),
		OPTION_GROUP(RawIcon.circle_o),
		NEXT(RawIcon.arrow_circle_right),
		PREVIOUS(RawIcon.arrow_circle_left),
		CODE_EDITOR(RawIcon.edit),
		TRANSLATE(RawIcon.refresh),
		ANIMATION_CONTROLS(RawIcon.play_circle),
		NEW_ARRAY_QUESTIONS(RawIcon.question),
		ATTACH(RawIcon.plus_circle),
		DETACH(RawIcon.minus_circle),
		ANSWER(RawIcon.edit),
		EXPLANATION(RawIcon.comment),
		VARIABLES(RawIcon.cogs),
		EXERCISE_POOL(RawIcon.hdd_o),
		PEER_REVIEW(RawIcon.group),
		VILLE_CS(RawIcon.flask),
		KEY(RawIcon.key),
		STACK(RawIcon.align_justify),
		SHARED_MEMORY(RawIcon.share),
		MATH_PERFORMANCE(RawIcon.dashboard),
		CATEGORY(RawIcon.list_ol),
		FIGHTER_JET(RawIcon.fighter_jet),
		NUMBER_LINE_TAG(RawIcon.times),
		VISUALIZE(RawIcon.camera),
		NEW_EXERCISE(RawIcon.plus_square_o),
		NEW_MESSAGE(RawIcon.send_o),
		RESULT(RawIcon.check),
		TERM_RANGE(RawIcon.arrows_h),
		OPERATORS(RawIcon.square),
		AMOUNT(RawIcon.database),
		DECREASE(RawIcon.minus),
		INCREASE(RawIcon.plus),
		ACHIEVEMENTS(RawIcon.trophy),
		MORE(RawIcon.plus),
		HOME_PAGE(RawIcon.home),
		MATH(RawIcon.subscript),
		UNDERLINE(RawIcon.underline),
		AUTOMATIC_GRADED(RawIcon.magic),
		CONSTRUCT(RawIcon.cogs),
		POOL(RawIcon.database),
		MANUAL_GRADED(RawIcon.edit),
		FOLDER_OPEN(RawIcon.folder_open),
		FOLDER_FILL(RawIcon.folder),
		DEMONSTRATION_EXER(RawIcon.suitcase),
		STUDY_JOURNAL_EXERCISE(RawIcon.book),
		MANUAL_COURSE_ASSIGNMENT(RawIcon.copy),
		MANUAL_EXAM(RawIcon.book),
		VIDEO(RawIcon.film),
		YOUTUBE(RawIcon.youtube),
		GEOGEBRA(RawIcon.superscript),
		OTHER_MATERIAL(RawIcon.paperclip),
		LECTURE(RawIcon.university),
		POLL(RawIcon.bar_chart_o),
		SDEDITOR(RawIcon.indent),
		WALL(RawIcon.comment),
		UNDO(RawIcon.undo),
		REDO(RawIcon.repeat),
		OPEN_POLL(RawIcon.plus),
		CLOSE_POLL(RawIcon.minus),
		FORUM(RawIcon.comment_o),
		START(RawIcon.play),
		STETHOSCOPE(RawIcon.stethoscope),
		NORTH(RawIcon.arrow_up),
		SOUTH(RawIcon.arrow_down),
		EAST(RawIcon.arrow_right),
		WEST(RawIcon.arrow_left),
		TURN_NORTH(RawIcon.arrow_circle_up),
		TURN_SOUTH(RawIcon.arrow_circle_down),
		TURN_EAST(RawIcon.arrow_circle_right),
		TURN_WEST(RawIcon.arrow_circle_left),
		IS_ROAD(RawIcon.road),
		LOOP(RawIcon.refresh),
		IF(RawIcon.question_circle),
		IFELSE(RawIcon.random),
		INFO_CIRCLE(RawIcon.info_circle),
		TAKE_PHOTO(RawIcon.camera),
		PH_MEASUREMENT(RawIcon.balance_scale),
		INCORRECT(RawIcon.close),
		LUMAMOBILE(RawIcon.map),
		MOVE_ASSIGNMENT(RawIcon.exchange);
		
		private final RawIcon rawIcon;
		private final Color color;
		private final IconVariant[] variants;
		
		private Icon(RawIcon icon, Color color, IconVariant... iconVariants) {
			rawIcon = icon;
			this.color = color;
			variants = iconVariants;
		}
		
		private Icon(RawIcon icon, IconVariant... iconVariants) {
			this(icon, null, iconVariants);
		}
		...
		@Override
		public String getHtml() {
			return getIcon().variant(null, color, variants);
		}
	...
	public enum OldIcon {
		 ...
		 * The file type is first checked by the MIME type provided. If no matches are found, the filename extension is checked next.<br>
