...
public class FocusChangingTextFieldExtension extends AbstractExtension {
	...
	public static FocusChangingTextFieldExtension[] extend(int changeAfter, TextField ... fields) {
		...
		for(int i = 0; i < fields.length; i++) {
			...
			extensions[i] = extend(fields[i], fields[(i+1) % fields.length], fields[prev], changeAfter);
		}
		return extensions;
	}
	/**
	 * 
	 * @param changeAfter
	 * @param fields where 0,0 is the top left corner, x coordinate is the first coordinate and y the second
	 * @return
	 */
	public static FocusChangingTextFieldExtension[][] extend(int changeAfter, TextField[][] fields) {
		if(fields.length == 0) {
	...
	public static FocusChangingTextFieldExtension extend(TextField field, Component nextComponent) {
		return extend(field, nextComponent, null, field.getMaxLength());
	}
	
	public static FocusChangingTextFieldExtension extend(TextField field, Component nextComponent, Component previousComponent, int changeAfter) {
		return extend(field, nextComponent, previousComponent, null, null, changeAfter);
	}
	
	public static FocusChangingTextFieldExtension extend(TextField field,Component nextComponent, Component previousComponent, Component upComponent, Component downComponent, int changeAfter) {
